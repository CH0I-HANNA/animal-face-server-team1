...
public class StorageController {
    ...
    @Operation(summary = "S3 Presigned URL 발급", description = "파일명과 Content-Type으로 PUT용 Presigned URL을 발급합니다.")
    @PostMapping("/presigned-url")
    public ApiResponse<PresignedUrlRes> getPresignedUrl(@RequestBody PresignedUrlReq req) {
        return ApiResponse.success(storageService.generatePresignedUrl(req));
    }
}
